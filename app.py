...
MAX_UPLOAD_MB       = 10
IMG_MAX_PX          = 1200   # resolução máxima guardada — suficiente para PDF A4
IMG_QUALIDADE       = 82     # qualidade JPEG no armazenamento
THUMB_MAX_PX        = 400    # resolução do preview na UI
...
def gerar_pdf(itens, header_bytes, progress_callback=None):
    buf = io.BytesIO()
    ...
    total_paginas = sum(
        -(-len(lista) // 2) for lista in sessoes.values()
    )
    pagina_atual = 0
    ...
    for sessao, lista in sessoes.items():
        for i in range(0, len(lista), 2):
            ...
            if progress_callback:
                progress_callback(pagina_atual / max(total_paginas, 1))
        ...
        for global_idx, item in enumerate(st.session_state.itens):
            ...
            if uploaded is not None:
                if uploaded.size > MAX_UPLOAD_MB * 1024 * 1024:
                    ...
                    raw        = uploaded.read()
                    processado = processar_imagem_upload(raw)   # ← comprime na entrada
    ...
    if st.button("📄 Gerar PDF", type="primary", use_container_width=True, disabled=len(validos) == 0):
        try:
            ...
            def atualizar_progresso(pct):
                barra.progress(pct, text=f"Gerando PDF… {int(pct * 100)}%")

            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(gerar_pdf, validos, header_bytes, atualizar_progresso)
                pdf_buf = future.result(timeout=180)
            ...
            st.error("Tempo limite excedido. Tente com menos imagens ou reduza o tamanho delas.")
